...
import com.myth.earth.develop.utils.XmlCompareUtil;
 ...
 **/
@Tool(category = ToolCategory.DIFF, name = "Xml差异", description = "比较xml文件key、value差异")
public class XmlDiffViewImpl extends AbstractToolView {
    ...
    public XmlDiffViewImpl(@NotNull Project project) {
        super(project);

        result1 = createTextArea();
        result1.getEmptyText().setText("仅展示Xml1中存在的内容");

        result2 = createTextArea();
        result2.getEmptyText().setText("仅展示Xml2中存在的内容");
        ...
        JButton diffKeyButton = createButton(80, "差异键", e -> {
            try {
                CompareResult compareResult = XmlCompareUtil.parseAndCompare(sourceTextField.getText(), targetTextField.getText());
        ...
        sourcePanel.add(createLineLabelPanel(60, "Xml1", sourceTextField), BorderLayout.CENTER);
        sourcePanel.add(diffKeyButton, BorderLayout.EAST);

        JPanel targetPanel = new JPanel(new BorderLayout());
        targetPanel.add(createLineLabelPanel(60, "Xml2", targetTextField), BorderLayout.CENTER);
        ...
        JPanel centerPanel = FormBuilder.createFormBuilder()
                                        ...
                                        .addComponentFillVertically(createBoxLabelPanel("在Xml1中存在", createScrollPane(result1)), 10)
                                        .addComponentFillVertically(createBoxLabelPanel("在Xml2中存在", createScrollPane(result2)), 10)
                                        .getPanel();
    ...
    private TextFieldWithBrowseButton buildFileSelectFieldButton(@NotNull Project project) {
        ...
        fileChooserDescriptor.withFileFilter(virtualFile -> XmlCompareUtil.FILE_TYPE.equalsIgnoreCase(virtualFile.getExtension()));
