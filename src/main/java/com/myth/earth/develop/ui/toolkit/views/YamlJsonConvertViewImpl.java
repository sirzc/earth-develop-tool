...
import com.myth.earth.develop.utils.JsonYamlUtil;
...
public class YamlJsonConvertViewImpl extends AbstractToolView {
    ...
    public YamlJsonConvertViewImpl(@NotNull Project project) {
        ...
        JButton changeButton = createButton(80, "转换", e -> {
            ...
            try {
                boolean isJson = isJson(inputText);
                String result = isJson ? JsonYamlUtil.jsonConvertToYaml(inputText) : String.join("\n", JsonYamlUtil.yamlConvertToJson(inputText));
    ...
    private boolean isJson(String inputText) {
        if (inputText == null || inputText.isEmpty()) {
            return false;
        }
        try {
            JsonYamlUtil.jsonConvertToYaml(inputText);
            return true;
        } catch (Exception e) {
            return false;
        }
    ...
    private TextFieldWithBrowseButton buildFileSelectFieldButton(@NotNull Project project) {
        ...
        fileChooserDescriptor.withFileFilter(
                virtualFile -> YamlCompareUtil.FILE_TYPE.equals(virtualFile.getExtension()) || YamlCompareUtil.SHORT_FILE_TYPE.equals(
                        virtualFile.getExtension()) || JsonCompareUtil.FILE_TYPE.equalsIgnoreCase(virtualFile.getExtension()));
        // 自定义文件选择后的处理逻辑
