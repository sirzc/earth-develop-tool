...
public class MyEditorTextField extends EditorTextField {
    ...
    public static void setupTextFieldEditor(@NotNull EditorEx editor) {
        ...
        // 设置边框
        editor.getScrollPane().setBorder(IdeBorderFactory.createEmptyBorder(JBUI.emptyInsets()));
    }
    ...
    public void setText(@Nullable final String text, @NotNull final FileType fileType) {
        try {
            ...
            PsiFile psiFile = ReadAction.compute(() -> PsiDocumentManager.getInstance(getProject()).getPsiFile(document));
            if (psiFile != null) {
